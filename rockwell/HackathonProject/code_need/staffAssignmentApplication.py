...
class staffSchedulingProblem:
    ...
    def NoMoreThanFiveShiftPerWeek(self, staffShiftsDict):

        violations = 0

        for i in staffShiftsDict.keys():
            tmp_list = staffShiftsDict[i]

            if 15 - tmp_list.count(0) > 5:
                violations += 1
    ...
    def getCost(self, schedule):
        ...
        # a = k 
        # AvailabiliyViolation = self.AvailabiliyViolation(staffShiftsDict)
        OnlyAssignStaffWhenThereIsWork = self.OnlyAssignStaffWhenThereIsWork(staffShiftsDict)
        # a = k
        NoMoreThanFiveShiftPerWeek = self.NoMoreThanFiveShiftPerWeek(staffShiftsDict)
        ...
        hardContstraintViolations = 50*OnlyAssignStaffWhenThereIsWork + 100*TwoStaffLineOneThreeStaffsLinesTwoThreeFour + NoMoreThanFiveShiftPerWeek#+AvailabiliyViolation#+ consecutiveShiftViolations + staffsPerShiftViolations + shiftsPerWeekViolations           # calculate the cost of the violations:
